...
const validateRegistration = [
  ...
  body('phone')
    .optional()
    .trim()
    .matches(/^\+?[\d\s\-()]{7,20}$/).withMessage('Must be a valid mobile number.'),
  body('dateOfBirth')
    .optional()
    .isISO8601().withMessage('Must be a valid date of birth.'),
  body('nic')
    .optional()
    .trim()
    .matches(/^[0-9]{9}[VvXx]?$|^[0-9]{12}$/).withMessage('Must be a valid NIC number.'),
  body('permanentAddress')
    .optional()
    .trim()
    .isLength({ max: 500 }).withMessage('Permanent address must be at most 500 characters.'),
  body('currentAddress')
    .optional()
    .trim()
    .isLength({ max: 500 }).withMessage('Current address must be at most 500 characters.'),
  body('address')
    .optional()
    .trim()
  ...
  body('university')
    .optional()
    .trim()
    .isLength({ max: 200 }).withMessage('University must be at most 200 characters.'),
  body('degree')
    .optional()
    .trim()
    .isLength({ max: 200 }).withMessage('Degree must be at most 200 characters.'),
  body('yearOfStudy')
    .optional()
